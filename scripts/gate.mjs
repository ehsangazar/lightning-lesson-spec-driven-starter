 ...
 *   npm run gate -- 001-items-api    name it explicitly
...
function activeSpec(changed) {
  ...
  for (const file of changed ?? []) {
    ...
    // Directories starting with _ are scaffolding, not specs.
...
function checkPaths() {
  ...
  let allow = null
  if (spec) {
    const specFile = resolve(root, 'specs', spec, 'spec.md')
    if (!existsSync(specFile)) fail(`no spec at specs/${spec}/spec.md`, listSpecs())

    allow = fencedList(readFileSync(specFile, 'utf8'), 'allow')
    if (allow.length === 0) {
      fail(
        `specs/${spec}/spec.md has no \`\`\`allow fence`,
    ...
    console.log(
      c.dim(
        '        No spec directory in this diff, so only the deny fence was checked.\n' +
          ...
          '        Scaffold one with: /speckit.specify',
      ),
    )
  }
