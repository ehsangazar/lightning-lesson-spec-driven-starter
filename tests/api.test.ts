...
describe('API contract', () => {
  ...
  it('lists items', () => {
    ...
    expect(res.status).toBe(200)
    expect(res.body).toEqual({ items: [{ id: '1', name: 'first item' }] })
  })
  ...
  it('creates an item and returns it', () => {
    ...
    expect(created.body).toEqual({ item: { id: '1', name: 'widget' } })

    const listed = app(request({ path: '/items' }))
    expect(listed.body).toEqual({ items: [{ id: '1', name: 'widget' }] })
  })

  it('trims the name before storing it', () => {
    const app = createApp({ store: createStore() })
    const res = app(request({ method: 'POST', path: '/items', body: { name: '  spaced  ' } }))
    expect(res.body).toEqual({ item: { id: '1', name: 'spaced' } })
  })
